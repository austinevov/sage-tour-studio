...
import { TourData } from '../services/fetchTour';
...
class Reactor {
  ...
  private _floorplans: { floor: number; path: string }[];
  ...
  public floorplanSrc = (): string => {
    return this.floorplanByFloor(this._currentFloor).path;
  };
  ...
  public save = (): void => {
    const tourToken = this._token;
    Object.keys(this._panoramas).forEach((key: string) => {
      const panorama = this._panoramas[key];
      const panorama_id = panorama.id();
      const { x, y } = panorama.floorplanPosition();
      const panorama_position = [x, 0, y].toString();
      const panorama_floor = panorama.floor();
      const panorama_edges = panorama
        .edges()
        .map(panorama => panorama.id())
        .toString();
      const panorama_label = panorama.name();

      axios.post('/api/update-panorama', {
        tourToken,
        panorama_id,
        panorama_position,
        panorama_floor,
  ...
  public populate = (tourData: TourData): void => {
    const panoramas = tourData.panoramas;
    this._floorplans = tourData.floorplans;
    panoramas.forEach((panorama: Panorama) => {
      this._panoramas[panorama.id()] = panorama;
    });

    this._dropBay.react({ captured: this._panoramas });
    this._floorplanViewer.setFloorplanFromSrc(
      this.floorplanByFloor(this._currentFloor).path
    );
    this._tourPreview.initialize(tourData.token, panoramas);
    this._token = tourData.token;
  };

  private floorplanByFloor = (
    targetFloor: number
  ): { floor: number; path: string } => {
    const fp = this._floorplans.filter(({ floor }) => floor === floor)[0];

    if (!fp) {
      throw new Error('Invalid floor number!');
    }

    return { floor: targetFloor, path: fp.path };
  };
