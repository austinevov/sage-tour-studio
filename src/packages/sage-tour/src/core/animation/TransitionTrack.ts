...
import { THETA_OFFSET } from '../SageTourInternal';
...
export default class TransitionTrack extends AnimationTrack {
  ...
  public getDirectionBetweenPoints = (): THREE.Vector3 => {
    let dir: THREE.Vector3 = this.destinationPanorama
      .position()
      .sub(this.sourcePanorama.position())
      .normalize()
      .negate();

    dir = new THREE.Matrix4().makeRotationY(THETA_OFFSET).multiplyVector3(dir);
